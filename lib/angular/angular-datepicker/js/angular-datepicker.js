  ...
  angular.module('pickadate.utils', [])
    .factory('pickadateUtils', ['dateFilter', function(/*dateFilter*/) {
      return {
        ...
        dateRange: function(first, last, initial) {
          var date, i, _i, dates = [];
    ...
    .directive('pickadate', ['$locale', 'pickadateUtils', 'dateFilter', '$sce', function($locale, dateUtils, dateFilter, sce) {
      return {
            ...
            '<div class="datepicker dropdown-menu">' +
                '<div class="datepicker-days">'+
                    '<table class="table-condensed">'+
                        '<thead>'+
                            '<tr>'+
                                '<th class="prev" ng-click="changeMonth(-1)">‹</th>'+
                                '<th colspan="5" class="switch">{{dayViewShortDesc}}</th>'+
                                '<th class="next" ng-click="changeMonth(1)">›</th>'+
                            '</tr>'+
        ...
        link: function(scope, element, attrs, ngModel)  {
          var disabledDates = scope.disabledDates || [],
              currentDate   = scope.baseDate || new Date(),
              today         = dateFilter(currentDate, 'yyyy-MM-dd'),
              selectDate = {
                  date: dateFilter(scope.date || currentDate , 'yyyy-MM-dd')
              };
          ...
          scope.render = function(initialDate) {
            initialDate = new Date(initialDate.getFullYear(), initialDate.getMonth(), 1, 0);

            var currentMonth    = initialDate.getMonth() + 1,
              dayCount          = new Date(initialDate.getFullYear(), initialDate.getMonth() + 1, 0, 0).getDate(),
            ...
            for (var i = 0; i < allDates.length; i++) {
              ...
              if (date < dateUtils.keepDateOnly(scope.minDate) || date > dateUtils.keepDateOnly(scope.maxDate) || dateFilter(date, 'M') !== currentMonth.toString()) {
              ...
              if (selectDate && (dateFilter(date, 'yyyy-MM-dd') === selectDate.date)) {
                className += ' active';
                scope.setDate(selectDate);
              }

              dates.push({date: dateFilter(date, 'yyyy-MM-dd'), className: className});
            }
          ...
          ngModel.$render = function () {
            ...
            if ((date = ngModel.$modelValue) && (indexOf.call(disabledDates, date) === -1)) {
              currentDate = dateUtils.stringToDate(scope.baseDate || currentDate);
              today       = dateFilter(currentDate, 'yyyy-MM-dd');
            } else if (date) {
