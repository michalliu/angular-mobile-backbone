            ...
            '<div class="datepicker dropdown-menu">' +
                '<div class="datepicker-days">'+
                    '<table class="table-condensed">'+
                        ...
                        '<tbody>'+
                            '<tr>'+
                                '<td class="{{d.className}}" ng-class="{\'active\': date == d.date}" ng-click="setDate(d)" ng-repeat="d in datesBucket[0]">{{d.date | date:"d"}}</td>'+
                            '</tr>'+
                            '<tr>'+
                                '<td class="{{d.className}}" ng-class="{\'active\': date == d.date}" ng-click="setDate(d)" ng-repeat="d in datesBucket[1]">{{d.date | date:"d"}}</td>'+
                            '</tr>'+
                            '<tr>'+
                                '<td class="{{d.className}}" ng-class="{\'active\': date == d.date}" ng-click="setDate(d)" ng-repeat="d in datesBucket[2]">{{d.date | date:"d"}}</td>'+
                            '</tr>'+
                            '<tr>'+
                                '<td class="{{d.className}}" ng-class="{\'active\': date == d.date}" ng-click="setDate(d)" ng-repeat="d in datesBucket[3]">{{d.date | date:"d"}}</td>'+
                            '</tr>'+
                            '<tr>'+
                                '<td class="{{d.className}}" ng-class="{\'active\': date == d.date}" ng-click="setDate(d)" ng-repeat="d in datesBucket[4]">{{d.date | date:"d"}}</td>'+
                            '</tr>'+
                        '</tbody>'+
                    '</table>'+
                '</div>'+
        ...
        link: function(scope, element, attrs, ngModel)  {
          ...
          scope.render = function(initialDate) {
            ...
            for (i=0;i<35;i++) {
                if (i%7 === 0) {
                    bucket = [];
                    datesBucket.push(bucket);
                }
                bucket.push(dates[i]);
            }

            scope.datesBucket = datesBucket;
          };
